...
cols=('Hotel_Address','Review_Date','Hotel_Name','Reviewer_Nationality','Reviewer_Score','days_since_review')
...
plt.show()
needed=['days_since_review','Reviewer_Score','Hotel_Address','Review_Date','Hotel_Name','Reviewer_Nationality','Review_Total_Positive_Word_Counts','Review_Total_Negative_Word_Counts']
...
pickle.dump(regressor, open('Random_forest', 'wb'))
'''

trained_model=pickle.load(open("Random_forest",'rb'))
...
print('Mean Square Error for Random_Forest', metrics.mean_squared_error(np.asarray(y_test), y_prediction))
print("The achieved accuracy using Random_Forest is " + str(accuracy))
print('true score value for Random_Forest : ' + str(true_score_value))
print('Predicted score value for Random_Forest : ' + str(predicted_score_value))
