...
cols=('Hotel_Address','Review_Date','Hotel_Name','Reviewer_Nationality','Reviewer_Score','Tags','days_since_review')

data=Feature_Encoder_try(data,cols);
'''
corr = data.corr()
...
pickle.dump(gnb, open('Naive', 'wb'))

'''
trained_model=pickle.load(open("Naive",'rb'))
...
print('Mean Square Error for Naive_Bayes', metrics.mean_squared_error(np.asarray(y_test), y_prediction))
print("The achieved accuracy using Naive_Bayes is " + str(accuracy))
print('true score value for Naive_Bayes : ' + str(true_score_value))
print('Predicted score value for Naive_Bayes : ' + str(predicted_score_value))
